...
# Day 9
  ...
  - found generate_cached's prefill step still hardcoded temperature=1.0/top_k=None/
    ...
    found on Day 8, just missed on the very first sampled token
...
# Day 11
  - wrote bench.py: tokens/sec (naive vs cached) across generation lengths, KV-cache
    bytes (fp32/fp16/int8) across sequence length, reused the Day 7 loss curve
  - time_generation does a warmup pass + median of repeats, not a single cold run
  - important correction: the earlier informal Day 8 check (single, no warmup) showed
    cached LOSING (0.52x) -- that was measurement noise, not real. The proper
  ...
  - fp16/int8 ratio came out to a constant 1.60x at every sequence length (makes
    ...
    either) -- consistent with Day 10's 3.2x fp32/int8 finding, since fp16 is just
